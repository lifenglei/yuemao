...
import { deepClone, judgeCustomMessageType } from './imutil.js';
let app = '';
export default class IMEventHandler {
  ...
  onMsg(msg) {
    ...
    if (app.globalData.currentChatTo === account) { //当前正在与此账户会话，禁止更新最近会话unread
      app.globalData.subscriber.emit('UPDATE_RECENT_CHAT_ON_MSG', { account: account,custom:msg.custom, time: msg.time, text: msg.text, type }, true)
    } else {
      app.globalData.subscriber.emit('UPDATE_RECENT_CHAT_ON_MSG', { account: account,custom:msg.custom, time: msg.time, text: msg.text, type })
    }
