import { generateFingerGuessImageFile, generateBigEmojiImageFile, generateRichTextNode, generateImageNode, calcTimeHeader } from '../../utils/imutil.js'
import { deepClone, clickLogoJumpToCard } from '../../utils/imutil.js'
...
Page({
  ...
  onLoad: function (options) {
    data_id = options.chatTo
    if (JSON.stringify(app.globalData.receiveObj) == "{}"){
      receiveObj = app.globalData.zhubo
    }else{
      receiveObj = app.globalData.receiveObj
    }
    ...
    if (chatTo === appInfo['uid']) {
      ...
      console.log(receiveObj)
      console.log(app)
      getShield(receiveObj)
      wx.setNavigationBarTitle({
        title: receiveObj.nick,
      })
      let chatToLogo = receiveObj.portrait || this.data.defaultUserLogo;
      self.setData({
        chatToLogo,
        loginAccountLogo: app.globalData.userInfo.portrait || this.data.defaultUserLogo
      })
    }
    ...
    if (Object.keys(loginMessageList).length != 0) {
      ...
      for (let time in chatToMessageList) {
        ...
        let customMsgType = JSON.parse(chatToMessageList[time].custom).msg_type
        if (msgType === 'text' && customMsgType != 2) {
          tempArr.push({
            type: 'text',
            text: chatToMessageList[time].text,
            time,
            custom: chatToMessageList[time].custom,
            msg_type: customMsgType,
        ...
        } else if (msgType == 'text' && customMsgType==2){
          let cardText = JSON.parse(chatToMessageList[time].text)
    ...
    console.log(this.data.messageArr)
    // 重新计算所有时间
 ...
 */
  ...
  sendRequest(text) {
    ...
    let l = {};
    l.users={};
    l.users.receive = receiveObj
    l.users.sender = app.globalData.senderInfo
  l.msg_type=1;
  console.log(receiveObj)
    app.globalData.nim.sendText({
      scene: 'p2p',
      to: self.data.chatTo,
      custom: JSON.stringify(l),
      text,
      done: (err, msg) => {
        // 判断错误类型，并做相应处理
        if (self.handleErrorAfterSend(err)) {
        ...
        // 最后一个参数表示，不更新未读数
        app.globalData.subscriber.emit('UPDATE_RECENT_CHAT', { account: msg.to, custom: JSON.stringify(l), time: msg.time, text: msg.text, type: msg.type }, true)
        // 滚动到底部
        self.scrollToBottom()
      }
    })
  ...
  sendImageToNOS(res) {
    ...
    let l = {};
    l.users = {};
    l.users.receive = app.globalData.receiveObj
    l.users.sender = app.globalData.senderInfo
...
function getShield(obj){
  ...
  requests.getRequest(url, data).then(res=>{
      ...
      if(res.data.dm_error==0){
        if(res.data.to_shield==0){
          app.globalData.shield=0
        }else{
          app.globalData.shield=1
        }
      }
  })
